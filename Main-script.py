...
# Stepwise regression: Use greedy algorithm to group features up into n_covar_sets complementary groups.
...
class  stepwise_lm:
    ...
    def fit(self, train, y):
        ...
        for var in covar_pool:
            ci = 0
            # Declare a list to store the mse of covar_sets by adding var
            ...
            for covar_set in self.chosen_covar_sets:
                if len(covar_set) < self.max_nr_covars_per_set:
                    ...
                    # If covar_set is full, then keep its original mse
            ...
            if gains.max() > 0:
                temp = gains.argmax()
                # Add var to the covar_set which gain the largest mse improvement
                self.chosen_covar_sets[temp].append(var)
                best_mses_by_set[temp] = mses_per_candidate_covar[temp]

        csi = 0
        # Fit linear models with all the covar_sets
        for covar_set in self.chosen_covar_sets:
    ...
    def predict(self, data):
        ...
        for covar_set in self.chosen_covar_sets:
            ...
            csi += 1
        # Add predictions of all the linear models with different covar_sets to dataframe
        return data
...
l0_wslm =  stepwise_lm(n_covar_sets = wslm_n_covar_sets, max_nr_covars_per_set =
...
# Predicting on holdout dataset day by day
while True:
    oidx += 1
    test = obs.features
